...
void main()
{
    ...
    if( !( ( (info >> 6) & 1 ) == 1 ) ){ // We can two-three flip!
        ...
        // Try to write our badface index to the location of the tetra.
        atomicMax( tetraMarkFlip[ tetA ], id + 1 );
        atomicMax( tetraMarkFlip[ tetB ], id + 1);
        ...
        for(uint i = 0; i < 3; i++){
            if( ( ( (info >> (i + 3)) & 1 ) == 1) ){
                ...
                // For now, we just take the first one. Maybe later I'll deal with a second possible 3-2 flip, but that case is degenerate, so it doesn't matter.
                j = i; 
            }
        }
        ...
        if( !(j == 3) ){
            ...
            atomicMax( tetraMarkFlip[ tetA ], id + 1);
            atomicMax( tetraMarkFlip[ tetB ], id + 1);
            atomicMax( tetraMarkFlip[ tetC ], id + 1);
        }
    }
}