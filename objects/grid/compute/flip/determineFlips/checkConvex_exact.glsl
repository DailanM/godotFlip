...
layout(set = 0, binding = 7, std430) restrict coherent buffer ufIndeterminedConvexity {uint indeterminedConvexity[];  };
...
void main()
{
  ...
  uint indeterminedBadFace = indeterminedConvexity[ id ];

  // ------------------------------ INDX ------------------------------

  // We iterate over the bad faces.
  uint indeterminedActiveFace = badFaces[ indeterminedBadFace ];
  uint indeterminedFaceId = activeFace[ indeterminedActiveFace ];
  ...
  // Get the tetrahedra
  uint tetA = faceToTetra[ 2*indeterminedFaceId + 0 ]; // The tetra in which we are positively oriented.
  uint tetB = faceToTetra[ 2*indeterminedFaceId + 1 ]; // The tetra in which we are negatively oriented.
  ...
  uint info = flipInfo[ indeterminedBadFace ];
  ...
  for(uint i = 0; i < 3; i++){
    if( ( (info >> i) & 1 ) == 1 ){
        ...
        float faceOrient = sign( orient3drobust( u, v, Bfar, Afar) );
        ...
        info |= (1 << (3 + i)); // need to check this face for 3-2 flip
          ...
          // The star will still be convex then. I don't know if that's optimal, but it should almost never matter.
    }
  }

  flipInfo[indeterminedBadFace] = info; //(info >> 3); // We can optionally erase the indeterminant info, but idk if I want to.
}